...
void Change_Account_Status(u8 Account_Id[idLimit])
{
	...
	u8 index ;
	u8 Status ;
	u8 new_status[Status_limit];
	printf("Account Status : ");
	fflush(stdin);
	gets(new_status);
	value2 = strcmp(new_status , "Active");
	value3 = strcmp(new_status , "Restricted");
	value4 = strcmp(new_status , "Closed");
	if(value2 == 0){
		Status = 0 ;
	}
	if(value3 == 0){
	...
	for(u8 i = 0;i<client;i++){
		value1 = strcmp(user[i].Account_ID , Account_Id);
		if(value1 == 0){
			user[i].Account_Status = Status ;
			index = i ;
			break;											
	...
	printf("%d",user[index].Account_Status);
